...
class RealtimeDDPvMF_lcm
{
  public:
    RealtimeDDPvMF_lcm(shared_ptr<lcm::LCM>& lcm, 
        shared_ptr<RealtimeMF>& pRtmf, 
        shared_ptr<RtDDPvMF>& pRtDdpvMF,
        string channel="KINECT_FRAME",
        string outPath="");
    virtual ~RealtimeDDPvMF_lcm();
...
RealtimeDDPvMF_lcm::RealtimeDDPvMF_lcm(shared_ptr<lcm::LCM>& lcm,
    shared_ptr<RtDDPvMF>& pRtDdpvMF, 
    string channel,
    string outPath)
  : lcm_(lcm), pRtDdpvMF_(pRtDdpvMF), outPath_(outPath)
{
  if(channel.compare("KINECT_FRAME") == 0)
    lcm_->subscribe( "KINECT_FRAME",&RealtimeDDPvMF_lcm::rgbd_cb,this);
  else if(channel.compare("CAMERA") == 0)
    lcm_->subscribe( "CAMERA",&RealtimeDDPvMF_lcm::disparity_cb,this);
  else
    cout<<"channel "<<channel<<" unkown!!"<<endl;
  cout<<"connected to channel "<<channel<<endl;
};

RealtimeDDPvMF_lcm::~RealtimeDDPvMF_lcm()
{}

void RealtimeDDPvMF_lcm::rgbd_cb(const lcm::ReceiveBuffer* rbuf, const
    string& channel, const  kinect::frame_msg_t* msg)
{
  ...
  if (msg->image.image_data_format == kinect::image_msg_t::VIDEO_RGB)
  {
    ...
    memcpy(rgb_.data, &(msg->image.image_data[0]), msg->image.width *
        msg->image.height * 3);
  }else if (msg->image.image_data_format ==
      kinect::image_msg_t::VIDEO_RGB_JPEG){
    ...
    cout << "Format not recognized: " << msg->image.image_data_format
      << endl;
  ...
  {
    cout << "MM depth image"<<msg->depth.height<<"x"
      << msg->depth.width<<endl;
  ...
  pRtDdpvMF_->compute(d_);
  visualizeNormals(); 
  cv::waitKey(10);
}

void RealtimeDDPvMF_lcm::disparity_cb(const lcm::ReceiveBuffer* rbuf,
    const string& channel, const  bot_core::images_t* msg)
{
  ...
  }else if (msg->images[0].pixelformat ==
      bot_core::image_t::PIXEL_FORMAT_GRAY )
  {
    ...
    memcpy(gray.data, &(msg->images[0].data[0]),
      ...
      bot_core::image_t::PIXEL_FORMAT_MJPEG )
  {
    cout<<"jpeg image "<<msg->images[0].size<<" "
      <<msg->images[0].width <<" "<<msg->images[0].height<<endl;
    rgb_ = cv::Mat(msg->images[0].height, msg->images[0].width, CV_8UC3);
  ...
  if (msg->image_types[1] == bot_core::images_t::DISPARITY_ZIPPED ) {
    ...
    unsigned long dlen = msg->images[1].width*msg->images[1].height*2 ;
    d_ = cv::Mat(msg->images[1].height, msg->images[1].width, CV_16SC1);
    uncompress(d_.data , &dlen, &(msg->images[1].data[0]),
        msg->images[1].size);
  } else{
...
//
  ...
  pRtDdpvMF_->compute(Z);
////  this->update();
////  cout<<pRtDdpvMF_->cRmf()<<endl;
////
 visualizeNormals(); 
 cv::waitKey(10);
}

void RealtimeDDPvMF_lcm::visualizeNormals()
{
  static int frameNr = 0;
  cout<<"visualize Normals"<<endl;
  cv::Mat Iseg = pRtDdpvMF_->overlaySeg(this->rgb_);
  cv::imshow("seg",Iseg);
  cv::Mat n = pRtDdpvMF_->normalsImg();
  cv::Mat d = pRtDdpvMF_->smoothDepthImg();
  cv::imshow("dS",d);
  cv::imshow("nS",n);
